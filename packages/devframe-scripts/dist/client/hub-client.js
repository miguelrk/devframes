...
(() => {
  ...
  var Immer2 = class {
    constructor(config2) {
      ...
      if (isBoolean(config2?.autoFreeze)) this.setAutoFreeze(config2.autoFreeze);
      if (isBoolean(config2?.useStrictShallowCopy)) this.setUseStrictShallowCopy(config2.useStrictShallowCopy);
      if (isBoolean(config2?.useStrictIteration)) this.setUseStrictIteration(config2.useStrictIteration);
    }
    createDraft(base) {
  ...
  var devframeId = config.id ?? "devframe-scripts";
  var VIEW_STORAGE_KEY = `${devframeId}-scripts-view`;
  ...
  var leafCount = (node) => node.kind === "leaf" ? 1 : node.children.reduce((sum, child) => sum + leafCount(child), 0);
  var addLeaf = (nodes, script) => {
    const name = script.id.split(":").at(-1) ?? script.id;
    const branch = nodes.find((node) => node.kind === "branch" && node.name === name);
    if (branch) {
  ...
  var addBranch = (nodes, name, path) => {
    const existing = nodes.find((node) => node.kind === "branch" && node.name === name);
    if (existing) return existing;
    const leafIndex = nodes.findIndex(
      (node) => node.kind === "leaf" && (node.script.id.split(":").at(-1) ?? node.script.id) === name
    );
    const branch = { kind: "branch", name, path, children: [] };
    if (leafIndex >= 0) {
  ...
  if (!mount) {
    ...
    void (async () => {
      ...
      const countLabel = () => {
        const rows = filtered();
        if (filter.trim()) return `${rows.length} matched \xB7 ${scripts.length} scripts in total`;
        return `${scripts.length} scripts in total`;
      };
      const renderLeaf = (script) => `
    ...
    `;
      ...
      const runActive = async () => {
        ...
        render();
        try {
          result = await rpc.call("run-script", { id: script.id });
        } catch (error) {
          result = {
            id: script.id,
            ok: false,
