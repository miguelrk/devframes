...
const templateMatches = (t: EmailTemplateEntry, q: string): boolean => {
...
const emailMeta = (id: string): string => {
...
const renderMainPanel = (): string => {
  ...
  if (selectedTab === 'email') {
    const panel = htmlBlobUrl
      ? `<div class="panel email">${emailMeta(selectedId)}<iframe src="${htmlBlobUrl}" title="Email preview"></iframe></div>`
      : `<div class="panel"><div class="empty">${rendering ? 'Rendering…' : 'No email yet — click Render'}</div></div>`
    return `${tabs}${panel}`
  }
