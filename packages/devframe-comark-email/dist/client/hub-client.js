...
`;
  ...
  var SCOPE = globalThis.__DEVFRAME_CONFIG__?.id ?? "devframe-comark-email";
  var CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .email-meta dt { color: #71717a; flex: none; width: 72px; }
  .email-meta dd { margin: 0; color: #e5e7eb; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  iframe { width: 100%; height: 100%; border: none; background: white; flex: 1; min-height: 0; }
...
${FORM_CSS}`;
  ...
  var metaByTemplate = /* @__PURE__ */ new Map();
  var selectedId = "";
  var selectedTab = "email";
  var inputMode = "form";
  ...
  var emailMeta = (id) => {
    const meta = metaByTemplate.get(id);
    if (!meta?.subject && !meta?.previewText) return "";
    return `
    <dl class="email-meta">
      ${meta.subject ? `<div class="row-meta"><dt>Subject</dt><dd>${escapeHtml(meta.subject)}</dd></div>` : ""}
      ${meta.previewText ? `<div class="row-meta"><dt>Preview</dt><dd>${escapeHtml(meta.previewText)}</dd></div>` : ""}
    </dl>
  `;
  };
  var renderMainPanel = () => {
  ...
  `;
    ...
    if (selectedTab === "email") {
      const panel = htmlBlobUrl ? `<div class="panel email">${emailMeta(selectedId)}<iframe src="${htmlBlobUrl}" title="Email preview"></iframe></div>` : `<div class="panel"><div class="empty">${rendering ? "Rendering\u2026" : "No email yet \u2014 click Render"}</div></div>`;
      return `${tabs}${panel}`;
    }
  ...
  var selectTemplate = async (id) => {
    await describeSelected(id).catch((err) => console.error("[comark-email] describe failed", err));
  ...
  var runAll = async () => {
    ...
    try {
      ...
      console.error("[comark-email] render-all failed", err);
    }
  };
  (async () => {
