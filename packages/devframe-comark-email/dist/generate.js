...
const fileNameFor = (id) => `${id.replaceAll('/', '-')}.html`;
/**
 * Render every host template (or a subset) to `{outDir}/{id}.html`.
    ...
    const workers = Array.from({ length: Math.min(limit, queue.length) || 1 }, async () => {
        while (queue.length) {
            ...
            try {
                ...
                log(`Wrote ${outPath} (${Buffer.byteLength(result.html)} bytes)${subjectNote}${warnNote}`);
            }
            catch (error) {
                fail++;
