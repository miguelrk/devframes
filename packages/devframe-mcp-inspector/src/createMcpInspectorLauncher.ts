...
const buildTransports = (options: CreateMcpInspectorLauncherOptions): McpTransport[] => {
  if (options.transports?.length) return options.transports
...
export const createMcpInspectorLauncher = (
  options: CreateMcpInspectorLauncherOptions,
): Plugin => {
