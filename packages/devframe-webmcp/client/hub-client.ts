...
const formatAnnotations = (annotations?: ModelContextTool['annotations']): string => {
...
if (!mount) {
  ...
  const resolveContext = async (): Promise<ResolvedContext & { tools: WebmcpSnapshot['tools'] }> => {
  ...
  const render = async () => {
    try {
      ...
      const rows = tools.length === 0
        ? '<tr><td colspan="4">No tools available yet.</td></tr>'
        : tools.map((tool) => {
      ...
      mount.innerHTML = `
        <header>
          ...
          <span>${escapeHtml(statusParts.filter(Boolean).join(' · '))}</span>
        </header>
        ${invokeBlock}
        <table>
          <thead>
  ...
  mount.addEventListener('click', (event) => {
    ...
    if (target.closest('[data-action="invoke"]')) {
      void (async () => {
        ...
        const textarea = mount.querySelector('#invoke-args') as HTMLTextAreaElement | null
